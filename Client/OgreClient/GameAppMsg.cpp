...
long CALLBACK TerrainNotice(int nFlag, int nSectionX, int nSectionY, unsigned long dwParam, MPTerrain* pThis)
{
	...
	// GameCharacter      *ChaList[MAX_MAP_SECTION_OBJ];
...
void CGameApp::MouseMove(int nOffsetX, int nOffsetY)
{
	...
	try
	{
		...
		if( _IsSceneOk() )
		{
			...
			{
				GameCharacter *pMain = GetCurScene()->GetMainCha();
...
void CGameApp::HandleSuperKey()
{
	...
	if(g_pGameApp->IsKeyDown(DIK_C))
	{
		...
		//	GameCharacter *pCha = GetCurScene()->GetMainCha();
		//	D3DXVECTOR3 vecCha = pCha->GetPos();
    ...
    else if( g_pGameApp->IsCtrlPress() )
    {
        ...
        {
            GameCharacter::IsShowLogName=!GameCharacter::IsShowLogName;
        }
...
void CGameApp::HandleContinueSuperKey()
{
	...
	if(IsKeyContinue(DIK_LEFT))
	{
		if(IsCameraFollow())
		{
			GameCharacter *pCha = _pCurScene->GetMainCha();
			if(pCha)
			{
...
const char* HandleMonsterCommand(string& strCmd, string &p1, string &p2)
{
	...
	if( strCmd=="load")
	{
		...
		while(!in.eof())
		{
			...
			GameCharacter *pCha = pScene->AddCharacter(nChaID);
			if(pCha)
			{
				pCha->setPos(x, y);
				pCha->setYaw(angle);
	...
	else if( strCmd=="save") // 保存放怪记录
	{
		...
		{
			GameCharacter *pCha = pScene->GetCha(i);
			if(pCha->IsValid())
			{
...
T_B
	...
	else if( strCmd=="testskilleffect" )
	{
		...
		GameCharacter* _pSelf = CGameScene::GetMainCha();
		GameCharacter* _pTarget = _pSelf;
		GameCharacter* _pAttack = _pSelf;
		lwMatrix44 mat;
		D3DXVECTOR3 pos;
	...
	else if( strCmd=="playallpose" )
	{
		...
		{
			GameCharacter* pCha = pMain->GetScene()->AddCharacter( nStart );
			if( pCha )
			{					
	...
	else if( strCmd=="createcha" )
	{
		...
		// 读id的表格
				...
				if( ((GameCharacterModel*)pCha)->LoadCha( pInfo->chModalType, pInfo->sModel, part_buf ) == 0 )
				{
			...
			else if( pInfo->chModalType == enumMODAL_BOAT )
			{
				...
				if( ((GameCharacterModel*)pCha)->LoadShip( pInfo->chModalType, pInfo->sModel, part_buf ) == 0 )
				{
					LG("error", RES_STRING(CL_LANGUAGE_MATCH_26), nScriptID, pInfo->szDataName);  
					continue;
				}
			...
			else if( pInfo->chModalType == enumMODAL_EMPL )
			{
				...
				if( ((GameCharacterModel*)pCha)->LoadTower( pInfo->chModalType, part_buf ) == 0 )
				{
					LG("error", RES_STRING(CL_LANGUAGE_MATCH_26), nScriptID, pInfo->szDataName);  
					continue;
				}
			...
			else if( pInfo->chModalType == enumMODAL_OTHER)
			{
				...
				if( ((GameCharacterModel*)pCha)->LoadCha( &load_info ) == 0 )
				{
					LG("error", RES_STRING(CL_LANGUAGE_MATCH_26), nScriptID, pInfo->szDataName);  
					continue;
				}  
			}
			if( ((GameCharacterModel*)pCha)->LoadPose( pInfo->sActionID ) == 0 )
			{
