...
inline static GameCharacter* GetCharacter( unsigned int nID, const char* error=NULL ) 
{
	if( !OgreApp::instance().GetCurScene()	) return NULL;

	GameCharacter*	pCha = OgreApp::instance().GetCurScene()->GetMainCha();

	if( pCha && pCha->GetGroupId()==nID) 
	{
        return pCha;
	}
	else
	{
		pCha = &OgreApp::instance().GetCurScene()->ma_Character.Get(nID);
	}
    if( !pCha && error )
    {
...
void NetActorMove( unsigned int id, stNetNotiMove& list )
{
	...
	GameCharacter * cha = CGameScene::GetMainCha();
...
void stNetActorCreate::SetValue( GameCharacter* pCha )
{
	/*
...
GameCharacter*	stNetActorCreate::CreateCha()
{
	/*
	...
	{
		GameCharacter*	pCha = g_stUIBoat.FindCha( ulWorldID );
		if( pCha )
		{
    ...
    GameCharacter* p = GetCharacter( ulWorldID );
    if( p )
    {
...
void stNetNPCShow::SetNpcShow( GameCharacter* pCha )
{
	/*
...
void NetActorDestroy( unsigned int nID, char chSeeType )
{
	...
	if( chSeeType==enumENTITY_SEEN_SWITCH ) 
	{
		GameCharacter*	pCha = g_stUIBoat.FindCha( nID );
		if( pCha )
		{
			pCha->SetHide( TRUE );
		}
	...
	// 消失
	GameCharacter* pCha = CGameApp::GetCurScene()->SearchByID(nID);	
	if( pCha )
	{
...
{
	/*
    GameCharacter * pCha = CGameApp::GetCurScene()->SearchByID( dwCharID );
    if( pCha )
    {
...
void NetActorSkillRep( unsigned int nID, stNetNotiSkillRepresent &SSkillRep)
{
	...
	else
	{
        ...
        if( !g_state )
        {
			...
			if( !p->IsAttackArea() )
			{
				GameCharacter* pFace = CGameApp::GetCurScene()->SearchByID(SSkillRep.lTargetID);
				if( pFace )
				{
...
void NetActorSkillEff(unsigned int nID, stNetNotiSkillEffect &SkillEff)
{
	...
	// 被攻击方
	GameCharacter* pTarget = pScene->SearchByID( nID );
	if( !pTarget ) 
	{
	...
	// 攻击方
	GameCharacter* pAttack = pScene->SearchByID( SkillEff.lSrcID );
	if( !pAttack ) 
	{
		// 如果在视野外,就有可能为空
	}
...
void NetActorLean(unsigned int nID, stNetLeanInfo &lean)
{
	...
	if( lean.chState==0 )
	{
		GameCharacter * cha = CGameApp::GetCurScene()->GetMainCha();
	...
	{
		GameCharacter * cha = GetCharacter(nID, "NetActorLean");
		if( cha )
		{
...
{
	/*
	GameCharacter * cha = GetCharacter( netsay.m_srcid, "NetSay" );
	if( !cha ) 
	{
    ...
    case enumITEM_APPE_MONS:        // 怪物掉落
        {
            if( info.lFromID )
            {
                GameCharacter* pFromCha = CGameApp::GetCurScene()->SearchByID(info.lFromID);
                if( pFromCha )
                {              
...
void NetChangeChaPart( GameCharacter* pCha, stNetLookInfo &SLookInfo )
{
	/*
...
void NetTempChangeChaPart( unsigned int nID, stTempChangeChaPart &SPart )
{
	/*
	// 临时协议，换装备
	GameCharacter * cha = GetCharacter( nID, "NetTempChangeChaPart" );
	if( !cha ) return;
...
{
	/*
	GameCharacter * pCha = GetCharacter(dwCharID, "NetStallName");
	if( pCha )
	{
...
void NetSynAttr( DWORD dwWorldID, char chType, short sNum, stEffect *pEffect )
{
    ...
    GameCharacter* pCha = CGameApp::GetCurScene()->SearchByID( dwWorldID );
    if( !pCha )
    {
...
void NetFace(DWORD dwCharID, stNetFace& netface, char chType)
{
	...
	{
		GameCharacter* pCha = GetCharacter(dwCharID, "NetSkillFace");
		if( !pCha ) return;
	...
	{
		GameCharacter* pCha = GetCharacter(dwCharID, "NetFace");
		if( !pCha ) return;
...
{
	/*
    GameCharacter* pCha = GetCharacter(dwChaID, "NetNpcStateChange");
    if( !pCha ) return;
...
{
	/*
    GameCharacter* pCha = GetCharacter(dwEntityID, "NetEntityStateChange");
    if( !pCha ) return;
	...
	case mission::TE_KILL:
		{
			...
			GameCharacter* pTarget = NULL;
			GameCharacter* pMain = CGameScene::GetMainCha();
			if( pMain )			
			{
...
{
	/*
    GameCharacter * pCha = CGameScene::GetMainCha();
	if( pCha ) 
	{
...
{
	/*
	GameCharacter * pCha = GetCharacter( lID );
	if( pCha )
	{
...
{
	/*
	GameCharacter * pCha = GetCharacter( lID, "NetChaEmotion" );
...
void stNetPKCtrl::Exec(GameCharacter* pCha)
{
	/*
...
{
	/*
	GameCharacter * pCha = GetCharacter( ulWorldID, "stNetPKCtrl" );
	if( pCha ) Exec( pCha );
	*/
}
...
void stNetDefaultSkill::Exec(void)
{
	...
	GameCharacter::SetDefaultSkill( pSkill );
	*/
}
...
{
	/*
	GameCharacter* pCha = GetCharacter( ulWorldID, "stNetUpdateHairRes" );
	if( !pCha ) return;
...
{
	/*
	GameCharacter* pCha = GetCharacter( lID, "NetChaSideInfo" );
	if( !pCha ) return;
...
void stNetAppendLook::Exec(unsigned long ulWorldID)
{
	/*
	// 目前仅考虑第一格
	GameCharacter* pCha = GetCharacter( ulWorldID, "stNetAppendLook" );
	if( !pCha ) return;
...
void stNetAppendLook::Exec(GameCharacter* pCha)
{
	/*
	if( !pCha ) return;
...
void NetItemUseSuccess(unsigned int nID, short sItemID)
{
	...
	GameCharacter * pCha = GetCharacter( nID );
	if( !pCha ) return;
...
{
	/*
	GameCharacter* pCha = CGameApp::GetCurScene()->SearchByHumanID(uiWorldID);
