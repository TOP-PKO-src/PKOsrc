...
*/
	...
	BeginWork(	GameConfig::instance().GetSendHeartbeat() );
...
void	NetIF::OnDisconnect(DataSocket *datasock,int reason)
{
    ...
    //	if( g_pGameApp )
    {
		...
		OgreApp::instance().SendMessage(APP_NET_DISCONNECT,reason);
...
void NetIF::SendPacketMessage(LPWPACKET pk)
{
	...
	{
		if ( !OgreApp::instance().GetCurScene() )
		{
			LG("error", "msgg_pGameApp->GetCurScene() Is NULL, Can't Send Socket Message!\n");
			return;
		}
		if (!OgreApp::instance().GetCurScene()->GetMainCha())
		{
