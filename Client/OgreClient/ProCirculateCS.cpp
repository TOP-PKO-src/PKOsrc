...
// 协议C->S : 发送行动信息
void CProCirculateCS::BeginAction( GameCharacter* pCha, DWORD type, void* param, CActionState* pState )
{
...
void CProCirculateCS::EndAction( CActionState* pState )
{
	...
	// log
	GameCharacter*	pMainCha = OgreApp::instance().GetCurScene()->GetMainCha();
	if (pMainCha)
	{
		LG(pMainCha->GetName(), "###Send(EndAction):\tTick:[%d]\n", GetTickCount());
	}
	//
}
