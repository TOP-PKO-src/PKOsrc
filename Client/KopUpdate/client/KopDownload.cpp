    ...
    : CFtpClient("kopdownload"), m_ServerList(), m_ServerListN()
{
	...
	// Modify by lark.li 20080715
    //m_strLoginUser = "kop";
    //m_strLoginPwd = "kop_usr";
    m_strLoginUser = "anonymous";
    m_strLoginPwd = "";
	// End
    m_strRunApp = "";
    m_bSkipUpdate = FALSE;
    ...
    {
        m_pMainWnd->GetDlgItem(IDREPAIR)->ShowWindow(SW_SHOW);
    }
	...
	g_KopData.SetTips("正在搜索最快的更新下载站点...");
	PostMessage(WM_KOPTIPS);
	CArcolPing icmp(3000);
	for (int i=0; i<nServerCnt; i++)
	{
		// Modify by lark.li 20080715 begin
		//m_ServerPingList.push_back(icmp.Ping((*pServerList)[i]));
	   m_ServerPingList.push_back(icmp.Ping((*pServerList)[i]));
		// End
	}		
...
void CKopDownload::OnLoadConfig(CString strKey, CString strValue)
{
    ...
    else if (strKey == "cooperate")
    {
        int cop = atoi(strValue.GetBuffer());
        ...
        if(cop != g_cooperate)
        {
            ...
            case COP_OURGAME:
                {
                    HBITMAP hBmp = LoadBitmap(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(IDB_DETAIL_OG));
                    m_pMainWnd->GetDlgItem(IDC_STATIC_AD)->SendMessage(STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hBmp);
                    m_pMainWnd->MessageBox("请使用联众游戏大厅来启动游戏！", "关闭客户端");
                }  break;
            case COP_SINA:
                {
                    HBITMAP hBmp = LoadBitmap(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(IDB_DETAIL_SINA));
                    m_pMainWnd->GetDlgItem(IDC_STATIC_AD)->SendMessage(STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hBmp);
                    //m_pMainWnd->MessageBox("请在游戏主页上登录后进入游戏。", "提示");
                    m_pMainWnd->ShowWindow(SW_HIDE);
                    MessageBox(0, "请在游戏主页上登录后进入游戏。", "提示", MB_OK);
                    ShellExecute(NULL, "open", "http://hdw.igame.sina.com.cn/", NULL, NULL, SW_SHOW);
                }  break;
            default:
                {
                    m_pMainWnd->MessageBox("请检查配置文件并使用相应的游戏大厅来启动游戏！", "关闭客户端");
                }  break;
            }
            ExitProcess(0);
        }
...
void CKopDownload::ChangeServer(WORD wSpeedClass /*从0开始*/)
{
	std::vector<CString>* pServerList = NULL;
	if (g_KopData.IsTelecom())
	{
	...
	int nIndex=-1;
	std::vector<int> pSpeedList = m_ServerPingList;
	for (WORD i=0; i<=wSpeedClass; i++)
	{
		nIndex=-1;
		int nFastestSpeedInThisClass=10000;
		for (int j=0; j<pSpeedList.size(); j++)
		{
			int nSpeed = pSpeedList[j];
			if (nSpeed != -1 && nSpeed < nFastestSpeedInThisClass)
			{
				nIndex = j;
				nFastestSpeedInThisClass = nSpeed;
			}
		}
		if (nIndex == -1) break;
		pSpeedList[nIndex]=-1;
	...
	if (nIndex>=0)
	{
		SetFtpServer((*pServerList)[nIndex]);
	}
	else
	{
		int nRand = int(rand() % nServerCnt);
...
BOOL CKopDownload::IsUpdateNeed()
{
	...
	//使用重连5次的方式替代原来只连接一次的方式，并允许玩家进入游戏 Arcol 2005-12-7
	bool bRet=false;
	for (int i=0; i < 5; i++)
	{
	...
	if (!bRet)
	{
		...
		PostMessage(WM_KOPTIPS);
		m_pMainWnd->MessageBox("更新服务器连接失败，您仍然可以进入游戏，若无法正常游戏，建议您去官网下载最新补丁!","自动更新");
		return FALSE;
	}
