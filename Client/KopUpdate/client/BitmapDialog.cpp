...
void CBitmapDialog :: MakeWindowRgn ()
{
	...
	else
	{
		...
		RECT *rcArry = NULL;
		if(file.Open("main.bin",CFile::modeRead)&&file.GetLength()!=0)
		//if(false)
		{
			int size = file.GetLength();
			int cout = size/sizeof(RECT);
			rcArry = new RECT[cout];
			file.Read(rcArry,size);
			
			for(int i=0;i<cout;i++)
			{
				CRgn rgnAdd;
				rgnAdd.CreateRectRgn (rcArry[i].left,
					rcArry[i].top, rcArry[i].right,rcArry[i].bottom);
		...
		}else
		{
			...
			if(!file.Open("main.bin",CFile::modeWrite|CFile::modeCreate))
			{
				MessageBox("error","open main.bin error!",MB_OK);
				assert(0);
			}
				...
				for (int x=0; x<width; x++)
				{
					...
					if (dcImage.GetPixel(x, y) == m_colTrans)
					{
						...
						else
						{
							...
							rgn.CombineRgn (&rgn, &rgnAdd, RGN_OR);
							RECT Trc;
							Trc.left = rcClient.left+row_start;
							Trc.top = rcClient.top+y;
							Trc.right = rcClient.left+x;
							Trc.bottom = rcClient.top+y+1;
							rcList.push_back(Trc);
			...
			for(std::vector<RECT>::iterator it = rcList.begin();it!=rcList.end();it++)
			{
				RECT Trect = *it;
				file.Write((char*)&Trect,sizeof(RECT));
			}
			file.Close();
		}
		SetWindowRgn (rgn, TRUE);
...
//bool CBitmapDialog::SaveData(const CRgn & rgn)
//{
//	CArchive v;
//	if (rgn.IsSerializable())
//	{
//		rgn.Serialize()
//
//	}
