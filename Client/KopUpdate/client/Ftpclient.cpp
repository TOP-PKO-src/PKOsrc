                    ...
                    DWORD dwContext /* = 1 */)
{
    ...
    try
    {
        if (m_pFtpc == NULL)
        {
            //m_pFtpc = GetFtpConnection(m_strFtpIp, m_strFtpUser, m_strFtpPwd, INTERNET_INVALID_PORT_NUMBER,bPassiveMode);
			m_pFtpc = GetHttpConnection(m_strFtpIp);
        }
    }
    ...
    try
    {
        //m_pRemoteFile = m_pFtpc->OpenFile(lpszRemoteFile, GENERIC_READ, dwFlags, dwContext);
		m_pRemoteFile = m_pFtpc->OpenRequest("GET", lpszRemoteFile, 0, 1, 0, HTTP_VERSION, INTERNET_FLAG_DONT_CACHE);
		if(!m_pRemoteFile->SendRequest())
		{
            // 连接要重新建立
            m_LocalFile.Close();
            m_pFtpc->Close();
