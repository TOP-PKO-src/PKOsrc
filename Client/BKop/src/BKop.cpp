...
//#pragma init_seg( lib )
pi_LeakReporter pi_leakReporter("Bkopmemleak.log");
CResourceBundleManage g_ResourceBundleManage("BKop.loc"); //Add by lark.li 20080130
...
BOOL CBKopApp::InitInstance()
{
	...
	{
		MessageBox(NULL,"读取道具数据失败!请检查文件及路径是否正确","错误",MB_OK);
	}
	if (!CServerMgr::LoadServerInfo())
	{
		MessageBox(NULL,"读取服务器列表数据失败!请检查文件及路径是否正确","错误",MB_OK);
		return FALSE;
	}
