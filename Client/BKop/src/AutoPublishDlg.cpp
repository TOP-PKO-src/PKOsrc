...
BOOL CAutoPublishDlg::OnInitDialog()
{
	...
	m_lstAutoPublishInfo.InsertColumn(0,"发布人",LVCFMT_CENTER,80);
	m_lstAutoPublishInfo.InsertColumn(1,"所在服务器",LVCFMT_CENTER,80);
	m_lstAutoPublishInfo.InsertColumn(2,"发送频道",LVCFMT_CENTER,70);
	m_lstAutoPublishInfo.InsertColumn(3,"开始时间",LVCFMT_CENTER,110);
	m_lstAutoPublishInfo.InsertColumn(4,"间隔分钟",LVCFMT_CENTER,65);
	m_lstAutoPublishInfo.InsertColumn(5,"重复次数",LVCFMT_CENTER,65);
	m_lstAutoPublishInfo.InsertColumn(6,"已发次数",LVCFMT_CENTER,65);
	m_lstAutoPublishInfo.InsertColumn(7,"当前状态",LVCFMT_CENTER,70);
	m_lstAutoPublishInfo.InsertColumn(8,"发布内容",LVCFMT_LEFT,500);

	m_cbxChannel.AddString("公告");
	...
	{
		MessageBox("警告:未发现可用角色!请先登陆游戏服务器!","自动发送管理器");
		OnCancel();
		return FALSE;
	}
...
void CAutoPublishDlg::OnBnClickedButtonAdd()
{
	...
	{
		MessageBox("错误:角色不存在或已删除,添加自动消息失败!","自动发送管理器");
		return;
	}
	CTime ct;
	CString temp;
	...
	{
		MessageBox("错误:参数不正确,添加自动消息失败!","自动发送管理器");
		return;
	}
...
void CAutoPublishDlg::OnRefresh()
{
	...
	for (int i=0;pData=g_AutoPublishMgr.GetAutoPublishData(i);i++)
	{
		...
		CString strTime=pData->GetStartTime().Format("%m月%d日 %H:%M:%S");
		...
		m_lstAutoPublishInfo.SetItem(i,2,LVIF_TEXT,"公告",0,0,0,NULL);
