...
BOOL CLoginPage::OnInitDialog()
{
	...
	m_LoginList.InsertColumn(1,"帐号",LVCFMT_LEFT,65);
	m_LoginList.InsertColumn(2,"登陆状态",LVCFMT_LEFT,150);
	m_LoginList.InsertColumn(3,"服务器地址",LVCFMT_LEFT,100);
	m_LoginList.InsertColumn(4,"在线人数",LVCFMT_LEFT,80);
	...
	m_ComboRoleSelect.AddString("第一个角色");
	m_ComboRoleSelect.AddString("第二个角色");
	m_ComboRoleSelect.AddString("第三个角色");
...
void CLoginPage::OnTimer(UINT nIDEvent)
{
	...
	if (nIDEvent==1)
	{
		...
		CString strTitle;
		strTitle.Format("BKop - 帐号统计(已登陆/总数):%d/%d个 玩家总人数统计(当前/最小/最大):%d/%d/%d人",nLoginAccountCount,nAccountCount,nTotalPlayerCount,nMinPlayerCount,nMaxPlayerCount);
...
int CLoginPage::AllocateIndex()
{
	...
	ASSERT(false);
	MessageBox("程序错误！客户端数量已达到程序内设上限，请重新启动程序");
	return -1;
}
...
void CLoginPage::OnBnClickedButtonAdd()
{
	...
	if (n!=CB_ERR)
	{
		...
		m_LoginList.SetItem(n,2,LVIF_TEXT|LVIF_STATE,"空闲",0,0,0,NULL);
...
void CLoginPage::OnBnClickedButtonSave()
{
	...
	if (n<=0)
	{
		MessageBox("操作失败：目前列表里没有帐号，请先添加帐号！","错误");
		return;
	}
...
void CLoginPage::OnBnClickedButtonLogin()
{
	...
	if (CPlayerMgr::GetTotalPlayers()>ACCOUNT_MAXNUM)
	{
		...
		{
			MessageBox("操作失败：超过最大用户，请选择浏览【当前的玩家】信息页后再登陆");
			return;
		}
	}
	for (int i=0;i<m_LoginList.GetItemCount();i++)
...
{
	// TODO: Add your control notification handler code here
	MessageBox("由于协议变动，此功能被屏蔽，请使用删除功能替代注销");
}
