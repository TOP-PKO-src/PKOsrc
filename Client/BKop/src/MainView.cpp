...
bool CMainView::InitView()
{
	...
	m_TabSystemInfo.InsertItem(0,"当前的玩家");
	m_TabSystemInfo.InsertItem(1,"选中的玩家");
	m_TabSystemInfo.InsertItem(2,"勾选的玩家");
	m_TabSystemInfo.InsertItem(3,"所有的玩家");
...
void CMainView::OnRefreshEdit()
{
	...
	do
	{
		index=-1;
		nowOrder=0;
		if (m_eTabSelect==CURRENT_PLAYER)	//当前的玩家
		{
		...
		{
			for (int i=0;i<=nTotal;i++)	//i从1起，0为系统消息
			{
				newOrder=0;
				if ( i==0 )
				{
				...
				if (m_eTabSelect==ALL_PLAYER)	//所有的玩家
				{
				...
				else if (m_eTabSelect==CHECKED_PLAYER)	//勾选的玩家
				{
				...
				else if (m_eTabSelect==SELECTED_PLAYER)	//选中的玩家
				{
	...
	*pResult = 0;
	...
	else
	{
		if (sel==ALL_PLAYER)
		{
			...
			{
				MessageBox("操作失败：超过最大个用户，不能查看多人信息");
				m_eTabSelect=CURRENT_PLAYER;
		...
		else if (sel==CHECKED_PLAYER)
		{
			if (CPlayerMgr::GetTotalPlayers()>ACCOUNT_MAXNUM)
			{
				MessageBox("操作失败：超过最大用户，不能查看多人信息");
				m_eTabSelect=CURRENT_PLAYER;
				m_TabSystemInfo.SetCurSel(0);
				return;
			}
