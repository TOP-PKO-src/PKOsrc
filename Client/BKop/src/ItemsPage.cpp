...
BOOL CItemsPage::OnInitDialog()
{
	...
	m_EquipList.InsertColumn(0,"位置",LVCFMT_LEFT,60);
	m_EquipList.InsertColumn(1,"装备",LVCFMT_LEFT,100);
	...
	m_ItemsList.InsertColumn(1,"道具",LVCFMT_LEFT,90);
	m_ItemsList.InsertColumn(2,"个数",LVCFMT_LEFT,40);
	m_ItemsList.InsertColumn(3,"类型",LVCFMT_LEFT,70);
...
void CItemsPage::ResetPage()
{
	...
	m_EquipList.InsertItem(0,"头盔");
	m_EquipList.InsertItem(1,"盔甲");
	m_EquipList.InsertItem(2,"手套");
	m_EquipList.InsertItem(3,"鞋子");
	m_EquipList.InsertItem(4,"项链");
	m_EquipList.InsertItem(5,"右手(剑)");
	m_EquipList.InsertItem(6,"左手(盾)");
	m_EquipList.InsertItem(7,"饰物(左)");
	m_EquipList.InsertItem(8,"饰物(右)");
	CString str;
...
void CItemsPage::RefreshPage()
{
	...
	pEquipItem=pSelf->GetHeadEquip();	// 0 头盔
	...
	pEquipItem=pSelf->GetBodyEquip();	// 1 盔甲
	if (pEquipItem && pEquipItem->GetItemID()>0 && pEquipItem->GetTotalNum()>0)
	{
	...
	pEquipItem=pSelf->GetGloveEquip();	// 2 手套
	if (pEquipItem && pEquipItem->GetItemID()>0 && pEquipItem->GetTotalNum()>0)
	{
	...
	pEquipItem=pSelf->GetShoesEquip();	// 3 鞋子
	if (pEquipItem && pEquipItem->GetItemID()>0 && pEquipItem->GetTotalNum()>0)
	{
	...
	pEquipItem=pSelf->GetNeckEquip();	// 4 项链
	if (pEquipItem && pEquipItem->GetItemID()>0 && pEquipItem->GetTotalNum()>0)
	{
	...
	pEquipItem=pSelf->GetSwordEquip();	// 5 右手(剑)
	if (pEquipItem && pEquipItem->GetItemID()>0 && pEquipItem->GetTotalNum()>0)
	{
	...
	pEquipItem=pSelf->GetShieldEquip();	// 6 左手(盾)
	if (pEquipItem && pEquipItem->GetItemID()>0 && pEquipItem->GetTotalNum()>0)
	{
	...
	pEquipItem=pSelf->GetLAccessoryEquip();	// 7 饰物(左)
	if (pEquipItem && pEquipItem->GetItemID()>0 && pEquipItem->GetTotalNum()>0)
	{
	...
	pEquipItem=pSelf->GetRAccessoryEquip();	// 8 饰物(右)
	if (pEquipItem && pEquipItem->GetItemID()>0 && pEquipItem->GetTotalNum()>0)
	{
