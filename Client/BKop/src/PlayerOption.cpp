...
bool CPlayerOption::Run()
{
	if (m_bAutoDrop_TestGateServer)
	{
		...
		{
			//CSystemInfo::Add(m_pPlayer->GetIndex(),"冲击组服务器功能启动，自动断线重连");
			m_pPlayer->GetPlayerMessages().AddMsg("冲击组服务器功能启动，自动断线重连");
	...
	if (m_bAutoDrop_TestAccountServer)
	{
		...
		{
			//CSystemInfo::Add(m_pPlayer->GetIndex(),"冲击帐号服务器功能启动，自动断线重连");
			m_pPlayer->GetPlayerMessages().AddMsg("冲击帐号服务器功能启动，自动断线重连");
			m_pPlayer->SetLoginState(CPlayer::eRESTART_LOGIN);
		}
	}
	if (m_bAutoDrop_TestGameServer)
	{
		...
		{
			//CSystemInfo::Add(m_pPlayer->GetIndex(),"冲击游戏服务器功能启动，自动断线重连");
			m_pPlayer->GetPlayerMessages().AddMsg("冲击游戏服务器功能启动，自动断线重连");
			m_pPlayer->SetLoginState(CPlayer::eRESTART_LOGIN);
		}
	}
	if (m_bAutoMove)
