...
function summon_monster(cnn, t)
  LG("lua_ai", "查找附近的怪, 类型", GetChaTypeID(cnn));
  ...
  for id, monster in m do
    if monster~=nil then
       ...
       LG("lua_ai", name, " 快来帮我, 对付目标:", GetChaDefaultName(t))
...
function tempt_monster(cha, m_type)
  LG("lua_tempt", "查找附近的怪, 类型", GetChaTypeID(cha));
  x,y = GetChaPos(cha)
  local m = {}
  ...
  for id, monster in m do
    if monster~=nil then
       ...
       LG("lua_tempt", name, " 有东西吃了, 快现身, 目标:", GetChaDefaultName(cha))
