...
{T_B
	...
	//	return FALSE;
	//}
	bool r = _connect.connect(g_Config.m_szDBIP, source, g_Config.m_szDBUsr, (char *)pswd.c_str(), err_info);
    if(!r)
    {
		LG("gamedb", "msgDatabase [%s] Connect Failed!, ERROR REPORT[%d]", source, err_info.c_str() );
        return FALSE;
    }
