...
bool TBLPersonInfo::GetInfo(int cha_id, stPersonInfo* pInfo)
{
	...
	if(_get_row(m_buf, 16, param, filter,&l_retrow))
	{
		if(l_retrow ==1 && get_affected_rows() ==1)
		{
			...
			pInfo->bShowMotto = atoi(m_buf[index++].c_str());
			strncpy(pInfo->szSex, m_buf[index++].c_str(), sizeof(pInfo->szMotto));
			pInfo->sAge = atoi(m_buf[index++].c_str());
			strncpy(pInfo->szName, m_buf[index++].c_str(), sizeof(pInfo->szMotto));
			strncpy(pInfo->szAnimalZodiac, m_buf[index++].c_str(), sizeof(pInfo->szMotto));
			strncpy(pInfo->szBloodType, m_buf[index++].c_str(), sizeof(pInfo->szMotto));
			pInfo->iBirthday = atoi(m_buf[index++].c_str());
			strncpy(pInfo->szState, m_buf[index++].c_str(), sizeof(pInfo->szMotto));
			strncpy(pInfo->szCity, m_buf[index++].c_str(), sizeof(pInfo->szMotto));
			strncpy(pInfo->szConstellation, m_buf[index++].c_str(), sizeof(pInfo->szMotto));
			strncpy(pInfo->szCareer, m_buf[index++].c_str(), sizeof(pInfo->szMotto));
			pInfo->iSize = atoi(m_buf[index++].c_str());
			pInfo->bPprevent = atoi(m_buf[index++].c_str());
