...
local cha_name=GetChaDefaultName(role)
	...
	SystemNotice ( role, "神秘的声音：<"..cha_name..">啊，今天让你见识一下真正的黑色死神的威力！" ) 
...
function map_copy_run_hell4( map_copy )
	...
	if CheckMonsterDead ( AZRAEL[18] ) == 1 then
		...
		if CRY[18]==0 then
			local Notice_all = "五楼守卫：愚蠢的人类啊，死神已经赐予我们不死的力量，我们还会再见面的，哇哈哈哈哈哈！！！"
	...
	if CRY[18]==1  then
		...
		if HELLCLOSETIME==300 then
			...
			if week ~= 7 then
				Notice("神秘的声音：没想到，人类居然成长到这个地步，连黑色的死神都成了废物，在我封印的时间内继续努力修练吧，下次见到你们的时候希望你们能更强大，哈哈哈哈哈哈！！")
			else
				Notice("神秘的声音：哈哈哈哈哈，终于，终于有人可以让我解放所有的力量了，来吧，来用你们的双眼见证真正的死亡游戏，见证真正的黑暗历史！")
			end				
		...
		for i = 1 , MAXNOTICE , 1 do
			if closetime ==NOTICETIME[i] then
				local Notice_all = "警告，距离地狱18层崩溃还有"..closetime.."秒！！请各位勇士赶快做好避难准备"
				MapCopyNotice ( map_copy , Notice_all )
			end
		end
