...
function after_create_entry(entry) 
    ...
    Notice("海盗广播：在沙岚新城外的["..posx..","..posy.."]位置附近发现一个通往《圣战》(65级以上)的入口！") --通知本组服务器的所有玩家
...
--    FinishSetMapEntryCopy(entry, 1) 
    ...
    local EntryName = "圣战入口: [海军阵营]  VS  [海盗阵营] (65级以上)"
...
function after_destroy_entry_guildwar(entry)
    ...
    Notice("海盗广播：通往《圣战》(65级以上)的入口已经消失了，祝您愉快～！") 
...
--    local BlueSideName = GetGuildName( BlueSide_GuildID ) 
    ...
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) --取地图入口的位置信息（地图名，坐标，目标地图名）
   ChaNotice(player_name, "海盗广播：在沙岚新城外的["..posx..","..posy.."]位置附近发现一个通往《圣战》(65级以上)的入口！") --通知本组服务器的所有玩家
...
local Team_In = IsInTeam(role)
	if Team_In == 1 then
		SystemNotice ( role , "您现在处于组队状态,不能进入圣战" )
		return 0
...
elseif Lv(role) < 65  then
	...
	SystemNotice(role, "进入此圣战的角色等级必须在65级以上")
	     return 0    
	end
...
--	end
	...
	SystemNotice ( role , "当前地图海军人数为"..count_haijun.."海盗人数为"..count_haidao)
	local Cha = TurnToCha(role)
	if  GetChaGuildID(Cha) <= 100 and GetChaGuildID(Cha) > 0 then
		if count_haijun < 45 then
	        
			SystemNotice(role,"进入圣战地图")
