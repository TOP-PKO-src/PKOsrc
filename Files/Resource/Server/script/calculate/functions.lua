...
function CheckIsValidTeam(character) --     -1 不是队长, -2 队伍不是3个人, -3  不是师徒
	...
	--SystemNotice( character , "报名失败"..ValidTeam)
	local team_num_now = GetAmphitheaterTeamCount()     ---竞技场 [[  取当前赛季队伍个数 ]] 
	if ValidTeam == -1 then
		SystemNotice( character , "报名失败，请确定您是当前队伍的队长")
		return LUA_FALSE
	end
	if ValidTeam == -2 then
	...
	if team_num_now >= 64 then
		...
		return LUA_FALSE	
	elseif ValidTeam == 1 and team_num_now < 64 then
		SystemNotice( character , "报名条件满足")	
		--team_num_now = team_num_now + 1
		--GiveItem(character , npc , 5848 , 1 , 4)		 
		--local role_csk = GetChaItem2 ( character , 2 , 5848 )
		--if role_csk ~= 1 then
			--SystemNotice( character , "操作失败，请确定您背包内只有一张参赛卡")
			--return LUA_FALSE		
		--else
			--SetItemAttr(role_csk , ITEMATTR_VAL_STR , team_num_now)
		return LUA_TRUE
		---end
	end	
end
---------------竞技场 [[  队伍注册 ]] -----------
-- function ArenaTeamSignUP(role,teamid,cha1,cha2,cha3) --p1[ 队伍ID ], p2[师傅ID], p3[队员1ID], p4[队员2ID]
	-- local csk_num = GetChaItem2 ( character , 2 , 5848 )
	-- local teamid = GetItemAttr(csk_num,ITEMATTR_VAL_STR)
	-- local cha1 = GetChaDefaultName ( character )
	-- local cha2 = GetChaDefaultName ( character )
	-- local cha3 = GetChaDefaultName ( character )
	-- csk_num = GetChaItem2 ( character , 2 , 5848 )
...
function CheckRealTeam(role)
	...
	csk_num = CheckBagItem(  role, 5848 )     --检测角色背包内是否有参赛卡
