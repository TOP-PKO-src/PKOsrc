...
SystemNotice ( role, "神秘的声音：不错啊，<"..cha_name..">，能够活着到达这里，如果你能够活着渡过眼前的弱水，我就承认你的能力！" ) 
...
function map_copy_run_hell2( map_copy )
	...
	if CheckMonsterDead ( AZRAEL[9] ) == 1 then
		...
		if CRY[9]==0 then
			local Notice_all = "咕咕：远古的诅咒已经启动，你们穿过这里必将唤醒你们体内最沉重的邪恶，愿死亡带给你们安息！！！"
	...
	if CRY[9]==1  then
		...
		if HELLCLOSETIME==300 then
			...
			Notice ("神秘的声音：干得不错啊，勇士们，连我最后一只宠物也被你们干掉了，但是，还记得前面的地狱发生什么了吗？这次，你们只有四分钟哦")
		...
		for i = 1 , MAXNOTICE , 1 do
			if closetime ==NOTICETIME[i] then
				local Notice_all = "警告，距离地狱9层崩溃还有"..closetime.."秒！！请各位勇士赶快做好避难准备"
				MapCopyNotice ( map_copy ,Notice_all )
			end
		end
