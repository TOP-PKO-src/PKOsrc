...
function after_create_entry(entry) 
    ...
    Notice("海盗广播：魔女之海的["..posx..","..posy.."]位置开放了通往《废灵之都》的漩涡。") --通知本组服务器的所有玩家
...
function after_player_login_abandonedcity(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) --取地图入口的位置信息（地图名，坐标，目标地图名）
    ChaNotice(player_name, "海盗广播：魔女之海的["..posx..","..posy.."]位置开放了通往《废灵之都》的漩涡。") --通知本组服务器的所有玩家
...
function check_can_enter_abandonedcity(role, copy_mgr)
	...
	if Lv(Cha) >=30 and Lv(Cha) <=45 then
		...
		SystemNotice(role, "进入废灵之都角色等级必须在30-45级之间")
		return 0    
	end
end
