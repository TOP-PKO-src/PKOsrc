...
function map_copy_run_secretgarden( map_copy )
	if PK_Win_CountNum == 60 then
		...
		if CheckMonsterDead ( RedSide_BaseRole ) == 1 then
			local Notice_all = "恭喜“["..GuildName_Blue.."]”公会在公会挑战赛中战胜了“["..GuildName_Red.."]”公会，事实证明了“["..GuildName_Blue.."]”公会才是真正的强者，让我们恭喜他们。"
			Notice ( Notice_all )
			PK_Win_CountNum = PK_Win_CountNum - 1
		...
		if CheckMonsterDead ( BlueSide_LCRole ) == 1 then
			if BlueSide_LCNotice == 0 then
				local Notice_all = "["..GuildName_Blue.."]的粮仓被摧毁，基地防御大幅度下降"
				Notice ( Notice_all )
		...
		if CheckMonsterDead ( RedSide_LCRole ) == 1 then
			if RedSide_LCNotice == 0 then
				local Notice_all = "["..GuildName_Red.."]的粮仓被摧毁，基地防御大幅度下降"
				Notice ( Notice_all )
		...
		if CheckMonsterDead ( BlueSide_DYKRole ) == 1 then
			if BlueSide_DYKNotice == 0 then
				local Notice_all = "["..GuildName_Blue.."]的弹药库被摧毁，基地攻击力大幅度下降"
				Notice ( Notice_all )
		...
		if CheckMonsterDead ( RedSide_DYKRole ) == 1 then
			if RedSide_DYKNotice == 0 then
				local Notice_all = "["..GuildName_Red.."]的弹药库被摧毁，基地攻击力大幅度下降"
				Notice ( Notice_all )
		...
		if Count == math.floor ( Count ) then
			local Notice_all = "恭喜“["..PK_JJHY_Winner.."]”公会在挑战赛中战胜了“["..PK_JJHY_Loser.."]”公会,《竞技花园》地图即将在"..PK_Win_CountNum.."秒内关闭"
			Notice ( Notice_all )
		end
		PK_Win_CountNum = PK_Win_CountNum - 1
