...
function AcceptMission( character, npc, missionlist, tpindex )
	...
	if tp == WORLD_MISSION then
		if Mission[id] == nil then
			SystemNotice( character, "AcceptMission:获取角色错误的世界任务索引信息！1ID = "..id )
		...
		if missionlist[id] == nil then
			...
			SystemNotice( character, "AcceptMission:获取角色错误的一般任务索引信息！2ID = "..id )
...
function CompleteMission( character, npc, missionlist, selitem, param )
	...
	if tp == WORLD_MISSION then
		if Mission[id] == nil then
			SystemNotice( character, "CompleteMission:获取角色错误的世界任务索引信息！3ID = "..id )
		...
		if missionlist[id] == nil then
			SystemNotice( character, "CompleteMission:获取角色错误的一般任务索引信息！4ID = "..id )
...
--			Notice("做任务奖励经验的防沉迷返还系数为"..retExpState)
		...
		elseif actions[i].func == GiveNpcMission then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:GiveNpcMission 失败！" )
				SystemNotice( character, "ActionProc:GiveNpcMission失败！" )
				return LUA_FALSE
			end
...
function TriggerProc( character, id, param1, param2, param3, param4 )
	...
	SystemNotice( character, "触发器OK了1！")
