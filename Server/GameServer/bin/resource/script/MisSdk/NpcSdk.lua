...
function MultiFunc( character, npc, page, funclist, count )
	...
	for id = 1, count, 1 do
		...
		elseif funclist[id].func == DuiHhuan1 then
			DuiHhuan1( character)
		elseif funclist[id].func == CheckIsValidTeam then
			CheckIsValidTeam( character)
		elseif funclist[id].func == ArenaTeamSignUP then
			ArenaTeamSignUP( character)
		elseif funclist[id].func == ArenaTeamCancel then
			ArenaTeamCancel( character)	
		elseif funclist[id].func == CheckIsValidRegTeam then
			CheckIsValidRegTeam( character)	
		else
...
function MsgProc( character, npc, rpk, page, trade, missionlist, exchangeData )
	...
	elseif usCmd == CMD_CM_FUNCITEM then --功能页选项命令信息处理
		...
		elseif item.func == OpenForge then
			...
			-- Add by lark.li 20080716 begin
		elseif item.func == OpenAmphitheater then			
			return OpenAmphitheater( character, npc )
			-- Eed
