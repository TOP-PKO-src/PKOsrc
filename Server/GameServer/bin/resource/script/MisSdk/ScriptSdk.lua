...
function AddMap( idname, str )
	...
	LG( "mission", "设置地图《"..MapList.mapname[MapList.count].."》，IDNAME = "..MapList.idname[MapList.count].."ID = "..MapList.count )
	PRINT( "设置地图《"..MapList.mapname[MapList.count].."》，IDNAME = "..MapList.idname[MapList.count].."ID = "..MapList.count )
	
	local ret = SetMap( MapList.idname[MapList.count], MapList.count )
	if ret == LUA_FALSE then
		LG( "mission", "设置地图信息失败《"..MapList.mapname[MapList.count].."》，IDNAME = "..MapList.idname[MapList.count].."ID = "..MapList.count )
		PRINT( "设置地图信息失败《"..MapList.mapname[MapList.count].."》，IDNAME = "..MapList.idname[MapList.count].."ID = "..MapList.count )
	end
end
...
function GetNpcInfo( npc, name )
	...
	if NpcMissionList.count > 0 then
		LG( "mission", "设置NPC携带任务标记！" )
		SetNpcHasMission( npc, 1 )
	end
	NpcPointer = LUA_NULL
	
	local str = "获取NPC《"..name.."》脚本数据信息，ID = "..NpcInfoList.count
	PRINT( str )
	LG( "npcinit", str )
end
...
function ModifyNpcInfo( npc, name, id )
	...
	PRINT( "设置npc脚本信息ID = ", id )
	--SetNpcScriptID( npc, id )
	if NpcMissionList.count > 0 then
		PRINT( "mission", "设置NPC携带任务标记！" )
		SetNpcHasMission( npc, 1 )
	else
		PRINT( "mission", "设置NPC不携带任务标记！" )
		SetNpcHasMission( npc, 0 )
	end
	
	NpcPointer = LUA_NULL
	
	local str = "修改NPC《"..name.."》脚本数据信息，ID = "..id
	PRINT( str )
	LG( "npcinit", str )
end
...
function NpcProc( character, npc, rpk, id )
	...
	if NpcInfoList[id] == nil then
		PRINT( "获取不到NPC的脚本信息！ID = ",  id )
		local npcname = GetCharName( npc )
		local str = npcname.."：你好，你找我有什么事？我现在比较忙。_．．．．．．．"
...
function NpcState( character, npcid, id )
   PRINT( "NpcState:character, npcid, NpcMissionList", character, npcid, id )
	if NpcInfoList[id] == nil or NpcInfoList[id].missionlist == nil then
		PRINT( "获取不到NPC的脚本信息！ID = ",  id )
		LG( "npc_error", "获取不到NPC的脚本信息！ID = ",  id )
		return LUA_FALSE
	end
...
function NpcInfoReload( name,  func )
	...
	PRINT( "获取到npc信息， pointer = , id = ", npc, id )
...
function SaleGoodsData( level, id, num, price, pricerange )
	if level == nil or id == nil or num == nil or price == nil or pricerange == nil then
		LG( "npcinit_error", "SaleGoodsData:函数参数错误！level, id, num, price, pricerange", level, id, num, price, pricerange )
		return
	end
...
function BuyGoodsData( level, id, num, price, pricerange )
	if level == nil or id == nil or num == nil or price == nil or pricerange == nil then
		LG( "npcinit_error", "BuyGoodsData:函数参数错误！level, id, num, price, pricerange", level, id, num, price, pricerange )
		return
	end
...
function UpdateGoodsData( tradenpc )
	...
	for n = 1, trade[1].count, 1 do
		...
		PRINT( "出售物品：ID = , Count = , CurPrice = ", trade[1].item[n].id, trade[1].item[n].count, trade[1].price[n].curprice )
	end
	...
	for n = 1, trade[2].count, 1 do
		...
		PRINT( "收购物品：ID = , Count = , CurPrice = ", trade[2].item[n].id, trade[2].item[n].count, trade[2].price[n].curprice )
	end
...
function SetTrigger( id, startup, event )	
	if Trigger[id] == nil then
		return LG( "trigger", "SetTrigger:错误的触发器索引ID = "..id )		
	end
...
function SetTriggerActionValue( id, index, p1, p2, p3, p4 )
	if id == nil or index == nil or TriggerList[id] == nil then
		PRINT( "SetTriggerActionValue:函数参数错误!triggerid = , index =", id, index )
		LG( "randmission_error", "SetTriggerActionValue:函数参数错误, triggerid = , index", id, index )
		return LUA_FALSE
	end
	
	if TriggerList[id].actions == nil or TriggerList[id].actions[index] == nil then
		PRINT( "SetTriggerActionValue:触发器没有动作信息错误!triggerid = , index = ", id, index )
		LG( "randmission_error", "SetTriggerActionValue:触发器没有动作信息错误!triggerid = , index = ", id, index )
		return LUA_FALSE
	end
...
function RegTrigger( id, triggerid )
	if id == nil or triggerid == nil then
		PRINT( "RegTrigger:注册的触发器ID不能为空！" )
	end
	if Trigger[triggerid] == nil then
		LG( "trigger_error", "RegTrigger:试图注册一个空的触发器到全局触发器列表中,ID = "..triggerid )
		return
	end
	if TriggerList[id] ~= nil then
		LG( "trigger_error", "RegTrigger:注册触发器时发现覆盖了原有触发器信息。ID = "..id )
	end
...
function SetNpcActive()
	...
	if ret ~= LUA_TRUE then
		PRINT( "SetNpcActive::设置当前NPC激活状态失败！NPC = "..GetCharName( NpcPointer ) )
		LG( "npcinit_error", "SetNpcActive::设置当前NPC激活状态失败！NPC = "..GetCharName( NpcPointer ) )
	end
end
...
function AddNpcMission( id )
	...
	PRINT( "设置任务["..NpcMissionList.count.."]：["..id.."]：《"..Mission[id].name.."》" )
	LG( "missioninit", "设置任务["..NpcMissionList.count.."]：["..id.."]：《"..Mission[id].name.."》" )
end
...
function DefineRandMission( id, name, misid, bounty, npcname, npcarea, leveltp )
	...
	if npcname ~= nil and npcarea ~= nil then
		...
		Mission[id].npcname = "未知NPC"
		Mission[id].npcarea  = "未知地图区域"
	end	
...
function InitTalkList()
	talklist.btalkstart = "接任务开始描述"
	talklist.btalkend = "接任务结束描述"
	talklist.rtalkstart = "还任务开始描述"
	talklist.rtalkend = "还任务结束描述"
	talklist.helpstart = "任务帮助开始描述"
	talklist.helpend = "任务帮助结束描述"
end
...
function AddRandMissionType( tp, tprand, talklist, exptp, randnum, p1, p2, p3, p4, p5, p6 )
	...
	for n = 1, Mission.curmission.missionlist.count, 1 do
		if Mission.curmission.missionlist[n].tp == tp then
			PRINT( "AddRandMissionType:随机任务类型添加重复，tp = "..tp )
			return
		end
	end
	...
	if tp == MIS_RAND_KILL then
		...
		TriggerAction( 2, SystemNotice, "金钱不足，取消该任务失败！" )
	...
	elseif tp == MIS_RAND_CONVOY then
		...
		TriggerAction( 1, HelpInfo, MIS_HELP_DESP, "谢谢你送我到这里，再见！" )
		TriggerAction( 1, RefreshCompleteFlag, Mission.curmission.sid )
		RegTrigger( p2, 1 )
		...
		TriggerAction( 2, HelpInfo, MIS_HELP_DESP, "谢谢你送我到这里，再见！" )
		TriggerAction( 1, RefreshCompleteFlag, Mission.curmission.sid )
		RegTrigger( p3, 2 )
		...
		TriggerAction( 3, HelpInfo, MIS_HELP_DESP, "谢谢你送我到这里，再见！" )
		TriggerAction( 1, RefreshCompleteFlag, Mission.curmission.sid )
		RegTrigger( p4, 3 )
		...
		TriggerAction( 4, HelpInfo, MIS_HELP_DESP, "谢谢你送我到这里，再见！" )
		TriggerAction( 1, RefreshCompleteFlag, Mission.curmission.sid )
		RegTrigger( p5, 4 )

		--SetTriggerType( 1, MIS_TRIGGER_RAND )
		local help = "执行任务《"
		help = help..Mission.curmission.name.."》已超过规定时间，任务失败！请从任务日志中清除该任务。"
...
function AddRandMissionInfo( id, level, tp, p1, p2, p3, p4, p5, p6, p7, p8 )	
	...
	if flag == 0 then
		PRINT( "AddRandMissionInfo:添加数据失败，因为该数据类型开关未打开。id, level, tp, p1, p2, p3, p4, p5, p6", id, level, tp, p1, p2, p3, p4, p5, p6 )
		LG( "randmission_error", "AddRandMissionInfo:添加数据失败，因为该数据类型开关未打开。", tp )
	end
	...
	elseif tp == MIS_RAND_EXPLORE then	--探索地图
		...
		PRINT( "AddRandMissionInfo:添加数据类型错误，tp = "..tp )
		LG( "randmission_init",  "AddRandMissionInfo:添加数据类型错误，tp = "..tp )
		return LUA_FALSE
	end
...
function AddRandKillInfo( level, monsterid, randvalue, randscope, exp, money )
	if Mission.curmission == nil or Mission.curmission.sid == nil then
		PRINT( "AddRandKillInfo:注册随机任务信息时，请先定义一个随机任务！level, monsterid,  randvalue, randscope, exp, money", level, monsterid, randvalue, randscope, exp, money )
		LG( "randmission_error", "AddRandKillInfo:注册随机任务信息时，请先定义一个随机任务！level, monsterid,  randvalue, randscope, exp, money", level, monsterid, randvalue, randscope, exp, money )
		return
	end
	
	local ret = AddRandMissionInfo( Mission.curmission.sid, level, MIS_RAND_KILL, monsterid, randvalue, randscope, exp, money )
	if ret ~= LUA_TRUE then
			PRINT( "AddRandKillInfo:AddRandMissionInfo:注册随机任务信息时出错！level, monsterid,  randvalue, randscope, exp, money", level, monsterid, randvalue, randscope, exp, money )
			LG( "randmission_error", "AddRandKillInfo:AddRandMissionInfo:注册随机任务信息时出错！level, monsterid,  randvalue, randscope, exp, money", level, monsterid, randvalue, randscope, exp, money )
			return
	end	
...
function AddRandGetItem( level, itemid, randvalue, randscope, exp, money )
	if Mission.curmission == nil or Mission.curmission.sid == nil then
		PRINT( "AddRandGetItem:注册随机任务信息时，请先定义一个随机任务！level, itemid,  randvalue, randscope, exp, money", level, itemid, randvalue, randscope, exp, money )
		LG( "randmission_error", "AddRandGetItem:注册随机任务信息时，请先定义一个随机任务！level, itemid,  randvalue, randscope, exp, money", level, itemid, randvalue, randscope, exp, money )
		return
	end
	
	local ret = AddRandMissionInfo( Mission.curmission.sid, level, MIS_RAND_GET, itemid, randvalue, randscope, exp, money )
	if ret ~= LUA_TRUE then
			PRINT( "AddRandGetItem:AddRandMissionInfo:注册随机任务信息时出错！level, monsterid,  randvalue, randscope, exp, money", level, itemid, randvalue, randscope, exp, money )
			LG( "randmission_error", "AddRandGetItem:AddRandMissionInfo:注册随机任务信息时出错！level, itemid,  randvalue, randscope, exp, money", level, itemid, randvalue, randscope, exp, money )
			return
	end
...
function AddRandSendInfo( level, npcid, exp, money )
	if Mission.curmission == nil or Mission.curmission.sid == nil then
		PRINT( "AddRandSendInfo:注册随机任务信息时，请先定义一个随机任务！level, npcid, exp, money", level, npcid, exp, money )
		LG( "randmission_error", "AddRandSendInfo:注册随机任务信息时，请先定义一个随机任务！level, npcid, exp, money", level, npcid, exp, money )
		return
	end
	
	if npcid == nil or NpcList[npcid] == nil or NpcList[npcid].mapid == nil or NpcList[npcid].areaid == nil then
		PRINT( "AddRandSendInfo:请填写正确的NPC ID 信息。npcid = ", npcid )
		LG( "randmission_error", "AddRandSendInfo:请填写正确的NPC ID 信息。npcid = ", npcid )
		return
	end
	...
	if ret ~= LUA_TRUE then
			PRINT( "AddRandSendInfo:AddRandMissionInfo:注册随机任务信息时出错！level, npcid, mapid, areaid, mapid, exp, money", level, npcid, NpcList[npcid].areaid, NpcList[npcid].mapid, exp, money )
			LG( "randmission_error", "AddRandSendInfo:AddRandMissionInfo:注册随机任务信息时出错！level, npcid, areaid, mapid, exp, money", level, npcid, NpcList[npcid].areaid, NpcList[npcid].mapid, exp, money )
			return
	end
...
function AddRandSendItem( level, item )
	if Mission.curmission.RandInfo[level] == nil then
		PRINT( "AddRandSendItem:添加送信随机任务可送物品，等级超出范围。level = ", level )
		LG( "randmission_error", "AddRandSendItem:添加送信随机任务可送物品，等级超出范围。level = ", level )
		return
	end
...
function AddRandConvoyInfo( level, charid, mapid, areaid, x, y, scope, exp, money )
	if Mission.curmission == nil or Mission.curmission.sid == nil then
		PRINT( "AddRandConvoyInfo:注册随机任务信息时，请先定义一个随机任务！level, npcid, areaid, mapid, exp, money", level, areaid, mapid, exp, money )
		LG( "randmission_error", "AddRandConvoyInfo:注册随机任务信息时，请先定义一个随机任务！level, npcid, areaid, mapid, exp, money", level, areaid, mapid, exp, money )
		return
	end

	local ret = AddRandMissionInfo( Mission.curmission.sid, level, MIS_RAND_CONVOY, charid, mapid, areaid, x, y, scope, exp, money )
	if ret ~= LUA_TRUE then
			PRINT( "AddRandConvoyInfo:AddRandMissionInfo:注册随机任务信息时出错！level, charid, mapid, mapid, areaid, x, y, scope, exp, money", level, charid, mapid, areaid, x, y, scope, exp, money )
			LG( "randmission_error", "AddRandConvoyInfo:AddRandMissionInfo:注册随机任务信息时出错！level, charid, mapid, areaid, x, y, scope, exp, money", level, charid, mapid, areaid, x, y, scope, exp, money )
			return
	end
...
function AddRandPrizeItem( level, item1, itemdata1, item2, itemdata2, item3, itemdata3, item4, itemdata4 )	
	...
	if Mission.curmission.RandInfo[level].LoopData[Mission.curmission.RandInfo[level].LoopData.count] == nil or Mission.curmission.RandInfo[level].LoopData[Mission.curmission.RandInfo[level].LoopData.count].Prize == nil then
		LG( "randmission_error", "AddRandPrizeItem:等级数据信息未初始化，misid = , level = ", Mission.curmission.sid, level )
		return
	end
...
function SetRandPrizeOdds( loopnum, odds, completenum )
	...
	if Mission.curmission.loopinfo[loopnum] ~= nil then
		PRINT( "SetRandPrizeOdds:数据设置重复。loopnum, odds completenum", loopnum, Mission.curmission.loopinfo[loopnum].odds, Mission.curmission.loopinfo[loopnum].completenum )
		LG( "randmission_error", "SetRandPrizeOdds:数据设置重复。loopnum, odds completenum", loopnum, Mission.curmission.loopinfo[loopnum].odds, Mission.curmission.loopinfo[loopnum].completenum )
	end
...
function AddNpcInfo( npcid, name, mapid, areaid )
	PRINT( "添加NPC，ID["..npcid.."]，名称《"..name.."》，MapID = "..mapid.."AreaID = "..areaid )
	LG( "npcinfo", "AddNpcInfo:npcid = "..npcid.."name = "..name.."mapid = "..mapid.."areaid = "..areaid )
	if NpcList[npcid] ~= nil then
		LG( "npcinfo", "AddNpcInfo:添加信息时发现重复ID，覆盖了原有NPC信息。ID = "..npcid.."name = "..NpcList[npcid].name )
		PRINT( "AddNpcInfo:添加信息时发现重复ID，覆盖了原有NPC信息。ID = "..npcid.."name = "..NpcList[npcid].name )
	end
	NpcList[npcid] = {}
...
function SetWoodResource( level, itemid, count, pileid )
	if level == nil or itemid == nil or count == nil or pileid == nil then
		PRINT( "SetWoodResource:函数参数错误！level, itemid, count, pileid", level, itemid, count, pileid )
		LG( "goods_error", "SetWoodResource:函数参数错误！level, itemid, count, pileid", level, itemid, count, pileid )
		return LUA_FALSE
	end
	
	if ResourceList == nil or ResourceList.wood == nil then
		PRINT( "SetWoodResource:资源信息列表不能为空！" )
		LG( "goods_error", "SetWoodResource:资源信息列表不能为空！" )
		return LAU_FALSE
	end
	
	if ResourceList.wood[level] ~= nil then
		PRINT( "设置木材资源打包信息时发现原有等级信息被覆盖！old data, level, itemid, count, pileid", level, itemid, count, pileid )
		LG( "goods_error", "设置木材资源打包信息时发现原有等级信息被覆盖！old data, level, itemid, count, pileid", level, itemid, count, pileid )
	end
	...
	PRINT( "设置木材资源打包信息：Level, ItemID, Count, PileID", level, itemid, count, pileid )
	LG( "packbag_init", "设置木材资源打包信息：Level, ItemID, Count, PileID", level, itemid, count, pileid )
	return LUA_TRUE
end

function SetMineResource( level, itemid, count, pileid )
	if level == nil or itemid == nil or count == nil or pileid == nil then
		PRINT( "SetMineResource:函数参数错误！level, itemid, count, pileid", level, itemid, count, pileid )
		LG( "goods_error", "SetMineResource:函数参数错误！level, itemid, count, pileid", level, itemid, count, pileid )
		return LUA_FALSE
	end
	
	if ResourceList == nil or ResourceList.mine == nil then
		PRINT( "SetMineResource:资源信息列表不能为空！" )
		LG( "goods_error", "SetMineResource:资源信息列表不能为空！" )
		return LAU_FALSE
	end
	
	if ResourceList.mine[level] ~= nil then
		PRINT( "设置矿石资源打包信息时发现原有等级信息被覆盖！old data, level, itemid, count, pileid", level, itemid, count, pileid )
		LG( "goods_error", "设置矿石资源打包信息时发现原有等级信息被覆盖！old data, level, itemid, count, pileid", level, itemid, count, pileid )
	end
	...
	PRINT( "设置矿石资源打包信息：level, ItemID, Count, PileID", level, itemid, count, pileid )
	LG( "packbag_init", "设置矿石资源打包信息：level, ItemID, Count, PileID", level, itemid, count, pileid )
	return LUA_TRUE
end

--添加港口信息
function AddBerthPort( id, name )
	PRINT( "添加港口信息：ID["..id.."],名称《"..name.."》" )
	LG( "boat_init", "添加港口信息：ID["..id.."],名称《"..name.."》" )
	if BerthPortList[id] ~= nil then
		LG( "boat_error", "AddBerthPort:添加港口信息时发现重复ID,被覆盖数据：ID = "..id.."name = "..BerthPortList[id].name )
		PRINT( "AddBerthPort:添加港口信息时发现重复ID,被覆盖数据：ID = "..id.."name = "..BerthPortList[id].name )
	end
...
function CreateBerthEntity( name, cid, infoid, xpos1, ypos1, dir1, berth, xpos2, ypos2, dir2 )
	PRINT( "创建船坞", name, cid, infoid, xpos1, ypos1, dir1, berth, xpos2, ypos2, dir2 )
	if name == nil or cid == nil or infoid == nil or xpos1 == nil or ypos1 == nil or dir1 == nil or berth == nil or xpos2 == nil or ypos2 == nil or dir2 == nil then
		PRINT( "CreateBerthEntity:创建函数参数信息错误!" )
		LG( "entity_error", "CreateBerthEntity:创建函数参数信息错误!" )
		return
	end
	...
	if ret ~= LUA_TRUE then
		PRINT( "CreateBerthEntity:GetCurSubmap函数调用失败！" )
		LG( "entity_error", "CreateBerthEntity:GetCurSubmap函数调用失败！" )
		return
	end
	local ret, e = CreateEventEntity( BERTH_ENTITY, submap, name, cid, infoid, xpos1, ypos1, dir1 )
	if ret ~= LUA_TRUE then
		PRINT( "CreateBerthEntity:CreateEventEntity:函数调用失败！tp, submap, name, cid, infoid, xpos1, ypos1, dir1", BERTH_ENTITY, submap, name, cid, infoid, xpos1, ypos1, dir1 )
		LG( "entity_error", "CreateBerthEntity:CreateEventEntity函数调用失败！tp, submap, name, cid, infoid, xpos1, ypos1, dir1", BERTH_ENTITY, submap, name, cid, infoid, xpos1, ypos1, dir1 )
		return
	end
	ret = SetEntityData( e, berth, xpos2, ypos2, dir2 )
	if ret ~= LUA_TRUE then
		PRINT( "CreateBerthEntity:SetEntityData函数调用失败！e, berth, xpos2, ypos2, dir2 ", e, berth, xpos2, ypos2, dir2 )
...
function CreateResourceEntity( name, cid, infoid, xpos, ypos, dir, itemid, count, time )
	PRINT( "创建资源实体", name, cid, infoid, xpos, ypos, dir, itemid, count, time )
	if name == nil or cid == nil or infoid == nil or xpos == nil or ypos == nil or dir == nil or itemid == nil or count == nil or time == nil then
		PRINT( "CreateResourceEntity:创建函数参数信息错误!" )
		LG( "entity_error", "CreateResourceEntity:创建函数参数信息错误!" )
		return
	end
	...
	local ret, submap = GetCurSubmap()
	if ret ~= LUA_TRUE then
		PRINT( "CreateResourceEntity:GetCurSubmap函数调用失败！" )
		LG( "entity_error", "CreateResourceEntity:GetCurSubmap函数调用失败！" )
		return
	end
	local ret, e = CreateEventEntity( RESOURCE_ENTITY, submap, name, cid, infoid, xpos, ypos, dir )
	if ret ~= LUA_TRUE then
		PRINT( "CreateResourceEntity:CreateEventEntity:函数调用失败！tp, submap, name, cid, infoid, xpos, ypos, dir", RESOURCE_ENTITY, submap, name, cid, infoid, xpos, ypos, dir )
		LG( "entity_error", "CreateResourceEntity:CreateEventEntity函数调用失败！tp, submap, name, cid, infoid, xpos, ypos, dir", RESOURCE_ENTITY, submap, name, cid, infoid, xpos, ypos, dir )
		return
	end
	ret = SetEntityData( e, itemid, count, time )
	if ret ~= LUA_TRUE then
		PRINT( "CreateResourceEntity:SetEntityData函数调用失败！e, itemid, count, time ", e, itemid, count, time )
...
function AddPfTable( curpf, uppf )
	...
	for n = 1, Profession[curpf].count, 1 do
		if Profession[curpf][n] == uppf then
			PRINT( "设置该职业转职列表信息已经存在，添加该信息失败！curpf = , uppf = ", curpf, uppf )
			LG( "PfTable_error", "设置该职业转职列表信息已经存在，添加该信息失败！" )
			return
		end
	end
...
function AddCatTable( cat, pf )
	...
	for n = 1, Category[cat].count, 1 do
		if Category[cat][n] == pf then
			PRINT( "设置该体形转职限制条件信息已经存在，添加该信息失败！, cat, pt ", cat, pf )
			LG( "PfTable_error", "设置该体形转职限制条件信息已经存在，添加该信息失败！" )
			return
		end
	end
...
function TestDefPage()
	...
	--第二页
	Talk( 2, "老奶奶：“没什么事，你忙你的吧”" )

	--第三页
	Talk( 3, "老奶奶：“有帮我去拿草芽么？记得要10个哦，精灵草就在白银城外面就有。”" )
	Text( 3, "好的我知道了", ct )
	...
	--第四页
	Talk( 4, "老奶奶：“记得拿好了药引要去找药草商人才行哦，他应该在白银城的星星酒吧附近。”" )
	Text( 4, "好的", MultiTrigger, GetMultiTrigger(), 2 )
	...
	--添加函数到函数列表中
	AddFuncList( CreditExchange, 0 ) --要钱
	Test( 4, "兑换钱", MultiFunc, GetFuncList(), GetNumFunc() )
	...
	--添加函数到函数列表中
	AddFuncList( CreditExchange, 1 ) --要物	
	Test( 4, "兑换物", MultiFunc, GetFuncList(), GetNumFunc() )
...
function TestDefMission( id, name, misid, scriptid, npcid, areaid )
	...
	MisBeginTalk( "谢谢你把我的包裹送过来！" )
	MisBeginCondition( AlwaysFailure )
	
	MisReultTalk( "谢谢你把我的包裹送过来！" )
	MisHelpTalk( "任务未完成, 请继续" )
...
function TestBorn()
	...
	TriggerAction( 1, SummonNpc, 1, 1, "药草商人·帝姆", 4 )
	RegTrigger( 25, 1 )
	
	--define trigger 26
	InitTrigger()
	TriggerAction( 1, SystemNotice, "Time single per 1 minute trig" )
	TriggerAction( 1, SystemNotice, "欢迎您来到海盗王的世界！" )
	RegTrigger( 26, 1 )
	
	--define trigger 27
	InitTrigger()
	TriggerAction( 1, SystemNotice, "Time cycle per 1 minute" )
	TriggerAction( 1, HelpInfo, MIS_HELP_DESP, "你是新手，好好学习，天天向上！" )
	TriggerAction( 1, HelpInfo, MIS_HELP_IMAGE, "新手地图指导手册.tga" )
