print("程序开始".."haha,resource\\script\\calculate\\")
...
CheckDmgChaNameTest [1] = "卡西斯"      
CheckDmgChaNameTest [2] = "我是垃圾"    
CheckDmgChaNameTest [3] = "CG毛毛"      
CheckDmgChaNameTest [4] = "大副度反对法"
...
--		Check_State ( ATKER , DEFER ) 
		if ValidCha(ATKER) == 0 then 
			LG ( "luascript_err" , "function Skill_Melee_End : ATKER 为空" ) 
			return 
		end 
		if ValidCha(DEFER) == 0 then 
			LG ( "luascript_err" , "function Skill_Melee_End : DEFER 为空" ) 
			return 
		end 
		...
		if dmgsa == 1 then
			...
			if Elf_Item ~= 0 then
				...
				if CheckElfSkill == 2 then
					...
					if ElfCrt == 1 then
						SystemNotice ( ATKER , "精灵发动爆击技能，攻击爆击！" )
						SystemNotice ( DEFER , "对方精灵发动爆击技能，攻击爆击！" )
						dmgsa = 2
						SetSus( DEFER , sus )
					end
				end
...
--		end 
		...
		if Check_Nianshou == 1 then
			...
			if Percentage == 1 then
				...
				SystemNotice ( ATKER , "得到神兽的庇佑，对手眩晕1秒")
			end
		end
...
function Skill_Range_End ( ATKER , DEFER , sklv )					--[[技能"远程普通攻击"的伤害计算（包括miss和暴击计算）]]--
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Range_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Range_End : DEFER 为空" ) 
		return 
	end 
		local ChaName_ATKER = GetChaDefaultName ( ATKER )
		local ChaName_DEFER = GetChaDefaultName ( DEFER )
...
--Notice("Phy_Dmg1")
	...
	local Can_Pk_Garner2 = Is_NormalMonster (b)
		...
		elseif map_name_ATKER == "teampk" or map_name_DEFER == "teampk" or map_name_ATKER == "starena2" or map_name_ATKER == "starena3" or map_name_DEFER == "starena2" or map_name_DEFER == "starena3" then 
			if Can_Pk_Garner2 == 0 then
			dmg = math.floor( phy_atk  * (1 - math.min (0.85 , phy_resist/100 )  ) - phy_def ) * 0.5
			return dmg
			end
		end
--	LuaPrint ( " 物理攻击＝ "..phy_atk.." 物理防御= "..phy_def.." 物理抵抗= "..phy_resist ) 
...
--	end 
	...
	--	Notice("22")
		...
		elseif map_name_ATKER == "teampk" or map_name_DEFER == "teampk" or map_name_ATKER == "starena2" or map_name_ATKER == "starena3" or map_name_DEFER == "starena2" or map_name_DEFER == "starena3" then
			if Can_Pk_Garner2 == 0 then
			dmg = Phy_Dmg ( atk, defer_def , defer_resist ) * 0.5
			end
		end
...
--		LG ("paodan"," skr_posx = ", skr_posx," skr_posy = ", skr_posy ) 
		if ValidCha( DEFER ) == 0 then 
			LG ( "luascript_err" , "fucntion Skill_Paodan_End : 炮弹攻击，传入被攻击者指针为空\n" ) 
			return 
		end 
...
function Skill_Ks_End ( ATKER , DEFER , sklv ) 
	...
	if Tree_ID == UnNormal_Tree_ID then
		if Tree_hp <= 800 then
			hpdmg = 0
			SystemNotice( ATKER , "看起来不会再出什么东西了，再砍树就倒了，让摇钱树也休息一下吧" )
		end
	end
...
function Skill_Wk_End ( ATKER , DEFER , sklv ) 
	...
	if KS_ID == UnNormal_KS_ID then						----判断是否是陨石
		...
		if ItemID_Use ~= 3908 and ItemID_Use ~= 3108 then					----如果不是合金矿镐不能挖
			SystemNotice( ATKER , "一定要使用合金矿稿才能挖掘" )
			return
		end
		...
		if Item_URE < 50 then						----耐久为0不能挖
			Take_Num = 0
			hpdmg = 0
			SystemNotice( ATKER , "矿镐已损坏，无法继续使用" )
		end
		...
		if KS_hp <= 800 then
			hpdmg = 0
			SystemNotice( ATKER , "看起来不会再出什么东西了，再挖就见底了，让陨石也休息一下吧" )
		end
...
function Skill_Xzfy_End ( ATKER , DEFER , sklv ) 
	...
	if Check_Heilong == 1 then
		local Percentage = Percentage_Random ( 0.7)
			if Percentage == 1 then
				...
				SystemNotice ( ATKER , "得到黑龙套装能力，技能加强")
			end
	end
	...
	if GetChaAIType( DEFER ) >= 21 then    --21以上为BossAI，如果是Boss根据boss不同，设定可以被加状态的次数
		if BOSSXZSJ[GetChaTypeID( DEFER )] == 0 then
			Notice ("身为Boss怎么可能被同一招式打倒，觉悟吧，勇者们！！")
			return
		else
...
function Skill_Hyz_End ( ATKER , DEFER , sklv ) 
	...
	--			Notice ( "dmg="..dmg)
		...
		elseif map_name_ATKER == "teampk" or map_name_DEFER == "teampk" or map_name_ATKER == "starena2" or map_name_ATKER == "starena3" or map_name_DEFER == "starena2" or map_name_DEFER == "starena3" then
			if Can_Pk_Garner2 == 0 then
				dmg = ( ( 1.5 + 0.1 * sklv ) ) * (math.min(3,( math.max ( 1, math.floor( aspd / 70  ) )  ))) * Atk_Dmg ( ATKER , DEFER ) * 0.5
				--Notice ( "dmg="..dmg)
			end
		end
	Hp_Endure_Dmg ( DEFER , dmg )  
...
--	SystemNotice ( role , "aaaaa" ) 
	SetRelive ( ATKER , DEFER ,  sklv , "玩家"..ChaName.."\n\n请求将您复活,您是否接受?" ) 
...
function Skill_BingX_End ( ATKER , DEFER , sklv ) 
	...
	if i <= 0 then
		SystemNotice ( ATKER , "召唤冰灵需要使用一个寒冰水晶")
		return
	end
...
function Skill_Mlch_End ( ATKER , DEFER , sklv ) 
	...
	if i <= 0 then
		SystemNotice ( ATKER , "催化需要使用一颗魔法草")
		return
	end
	local b = ( sklv - 1 ) * 0.05
	...
	elseif a == 1 then
		j = 1
		SystemNotice ( ATKER , "进入节能施法，催化时不消耗魔法草" )
	end
	if j == 1 then
		local statelv = sklv 
...
function Skill_Guz_End ( ATKER , DEFER , sklv )									--技能“鬼舞斩”的效果阶段
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Guz_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Guz_End : DEFER 为空" ) 
		return 
	end 
	local a = 1 
...
function Skill_Lj_End ( ATKER , DEFER , sklv ) 
	...
	if Check_Heilong == 1 then
			...
			if Percentage == 1 then
				  dmg =  dmg*1.5 
				SystemNotice ( ATKER , "得到黑龙套装能力，伤害加成")
			end
	end
...
function Skill_Ldc_End ( ATKER , DEFER , sklv ) 
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Ldc_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Ldc_End : DEFER 为空" ) 
		return 
	end 
...
function Skill_Xlcz_End ( ATKER , DEFER , sklv ) 
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Xlcz_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Xlcz_End : DEFER 为空" ) 
		return 
	end 
--	local atkdmg = Atk_Dmg ( ATKER , DEFER )
	...
	--			Notice ( "statetime="..statetime)
		...
		elseif map_name_ATKER == "teampk" or map_name_DEFER == "teampk" or map_name_ATKER == "starena2" or map_name_ATKER == "starena3" or map_name_DEFER == "starena2" or map_name_DEFER == "starena3" then
			if Can_Pk_Garner2 == 0 then
			hpdmg = math.floor (( 10 + sta_atker * 2 ) * ( 1 + sklv * 0.25 ) * ( 1 +  lv_dif * 0.025 )) * 0.5
			end
		end
...
function Skill_Bdj_End ( ATKER , DEFER , sklv ) 
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Bdj_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Bdj_End : DEFER 为空" ) 
		return 
	end 
...
--	end 
	...
	local Check_Heilong  = CheckItem_Heilong ( role1)
		if Check_Heilong == 1 then
			...
			if Percentage == 1 then
				  statetime =  statetime*2
				SystemNotice ( ATKER , "得到黑龙套装能力，持续时间加长")
			end
	end
	local hp_defer = Mxhp ( DEFER ) 
...
function Skill_Lxjy_End ( ATKER , DEFER , sklv ) 
	...
	local hp = Hp( DEFER ) 
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Lxjy_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Lxjy_End : DEFER 为空" ) 
		return 
	end 
...
function Skill_Lh_End ( ATKER , DEFER , sklv ) 
	...
	local hp = Hp( DEFER ) 
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Lh_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Lh_End : DEFER 为空" ) 
		return 
	end 
	dmg = ( 1+sklv*0.05 ) * Atk_Dmg ( ATKER , DEFER ) 
	Hp_Endure_Dmg ( DEFER , dmg )  
...
function Skill_Ctd_End ( ATKER , DEFER , sklv ) 
	...
	local hp = Hp( DEFER ) 
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Ctd_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Ctd_End : DEFER 为空" ) 
		return 
	end 
...
--	Notice ( "defer_type = "..defer_type) 
	...
	Notice ( "defer_hp = "..hp )
	if ValidCha(ATKER) == 0 then 
		LG ( "luascript_err" , "function Skill_Slzb_End : ATKER 为空" ) 
		return 
	end 
	if ValidCha(DEFER) == 0 then 
		LG ( "luascript_err" , "function Skill_Slzb_End : DEFER 为空" ) 
		return 
	end 
	dmg = 1500 
	Hp_Endure_Dmg ( DEFER , dmg )  
...
function Skill_PKXL_End ( ATKER , DEFER , sklv ) 
	...
	if i <= 0 then
		SystemNotice ( ATKER , "没有修理用木头啊，拿什么修理呢？")
		return
	end
	local j = DelBagItem(ATKER,4661,1)
	if j == 1 then
		SystemNotice ( ATKER , "修理中..." ) 
...
function Skill_Wudiyaoshui_End( ATKER , DEFER , sklv )
	...
	local message = cha_name.."进入无敌状态5秒"  
	Notice ( message )
end 
...
function Skill_JLFT_BEGIN( role , sklv )			
	...
	if item_elf_type ~= 59 or Part1~=1 then
		SkillUnable(role) 
		SystemNotice ( role , "此技能必须在装备新一代精灵的情况下才有实际效果！" ) 
		return 
	end
	...
	if item_elf_hp<5000 then
		SkillUnable(role) 
		SystemNotice ( role , "发动此技能精灵体力至少需要100点！" ) 
		return 
	end
...
function Skill_jlzb_Begin( role , sklv )	
	...
	if item_elf_hp<50 then
		SkillUnable(role) 
		SystemNotice ( role , "发动此技能精灵体力必须大于零！" ) 
		return 
	end
...
function State_5MBS_Add(role , sklv)
	...
	SystemNotice (role,"神对你的行为感到愤怒！接受天谴吧！")
end
...
function JLTX_usu(role)
	...
	if item_count <= 0 then    
		SkillUnable(role)
		SystemNotice ( role , "背包中没有使用技能所需的精灵硬币哦" ) 
		return 0
	end 
...
function Skill_Shenshenglazhu_End( ATKER , DEFER , sklv )
	...
	if Item_CanGet <= 0 then
		SystemNotice(ATKER ,"身上没有足够的空间，使用神圣蜡烛失败")
		UseItemFailed ( ATKER )
		return
	end
	local a = DelBagItem ( ATKER , 3006 , 1 ) 
	if a ~= 1 then
		SystemNotice(ATKER, "删除神圣蜡烛失败")
		return 0
	end
...
function Skill_Daibuzheng1_End ( ATKER , DEFER , sklv )
	...
	if Num_Haorenzheng >= 1 then
		...
		if Num_Liangminzheng >= 1 then
			...
			SetSpawnPos ( ATKER , "监狱岛" )
			MoveCity( ATKER , "监狱岛")
		else
			Notice ( defer_name.."恶贯满盈,被"..atker_name.."逮捕归案!判有期徒刑1小时!")
			SetSpawnPos ( DEFER , "监狱岛" )
			MoveCity( DEFER , "监狱岛")
		end
	end
end
