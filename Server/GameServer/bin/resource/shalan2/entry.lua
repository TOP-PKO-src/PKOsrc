...
function after_create_entry(entry) 
    ...
    Notice("海盗广播：有玩家发现，在魔女大陆的["..posx..","..posy.."]位置附近发现一个通往《幻影沙岚》的入口，该消息尚未核实，请有关单位注意。") --通知本组服务器的所有玩家

end

function after_destroy_entry_shalan2(entry)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
    Notice("海盗广播：据玩家提供的消息，通往《幻影沙岚》的入口已经消失了。请大家继续关注我台的广播，祝您愉快～！") 

end

function after_player_login_shalan2(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) --取地图入口的位置信息（地图名，坐标，目标地图名）
    ChaNotice(player_name, "海盗广播：有玩家发现，在魔女大陆的["..posx..","..posy.."]位置附近发现一个通往《幻影沙岚》的入口，该消息尚未核实，请有关单位注意。") --通知本组服务器的所有玩家
...
function check_can_enter_shalan2( role, copy_mgr )
	...
	if Lv(role) > 89 then
		SystemNotice(role, "进入进入幻影沙岚角色等级必须在90级以下")
		return 0    
	end
	...
	if Num < 1 then
		SystemNotice(role, "没有真实面具，无法正确通过幻像到幻影沙岚")	
		return 0
	end

	local Credit_Shalan2 = GetCredit(role)
	if Credit_Shalan2 < 10 then
		SystemNotice(role, "您的名声值不够,不能进入幻影沙岚")
		return 0
	else
		DelCredit(role,10)
		return 1
...
function begin_enter_shalan2(role, copy_mgr)
	...
	if Dbag == 1 then
		SystemNotice(role,"开始进入《幻影沙岚》") 
		MoveCity(role, "幻影沙岚")

	else
	
		SystemNotice(role, "收取真实面具失败，无法正确通过幻象到幻影沙岚")
	end
end